...
void* threadFunction(void *arg){
    ...
    while (flag == 0){
        ...
        int size = sprintf(buf, "%ju", (uintmax_t)path);
        int rv = mq_send(mq, buf, strlen(buf), 0);
        if (rv == -1) {
            perror("mq_send");
        } else {
            std::cout << "Written message: " << path << std::endl;
        }
        sleep(1);
    }
...
int main(){
    ...
    mq = mq_open(name,O_CREAT | O_WRONLY | O_NONBLOCK, S_IRUSR | S_IWUSR, &attr);
